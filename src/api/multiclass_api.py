...
"""
多分类违规内容检测API服务
基于FastAPI的RESTful API服务，支持多种违规类别检测
"""
...
def initialize_detector():
    ...
    try:
        ...
        if detector.load_models():
            logger.info("多分类检测器初始化成功")
        else:
            logger.error("多分类检测器初始化失败")
    except Exception as e:
        logger.error(f"初始化多分类检测器时发生错误: {e}")

@app.on_event("startup")
async def startup_event():
    """应用启动事件"""
    logger.info("正在启动多分类违规内容检测API服务...")
    initialize_detector()
...
async def health_check():
    ...
    return {
        ...
        "supported_categories": getattr(detector, 'class_labels', []) if detector else []
    }

@app.post("/detect", response_model=DetectionResponse)
async def detect_multiclass_content(request: DetectionRequest):
    """
    检测单个文本的违规类别
...
async def get_supported_categories():
    ...
    categories = getattr(detector, 'class_labels', [])
    return {
